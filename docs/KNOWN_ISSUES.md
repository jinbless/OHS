...
| 조문 | 유형 | 실제 매칭 결과 | 원인 |
|------|------|:-------------:|------|
| **제383조** (작업의 제한) | administrative | 제14조, 제42조, 제44조 | 관리적 조항으로 시나리오에서 직접 매칭 어려움 |
| **제356조** | compound_risk | 제16조, 제66조, 제73조 | 복합 위험 시나리오에서 정확 조문 특정 실패 |

### KOSHA GUIDE 6건 (93/99)

| 가이드 | 분류 | 원인 |
|--------|:----:|------|
| X-70-2016 (운전원 행동분석) | X | X분류 특수 가이드, 유사 가이드가 다른 분류에 존재 |
| P-10-2012 | P | 공정안전 가이드 혼동 |
| E-94-2011 (산업용 기계설비 전기장치) | E | 기계(M)와 전기(E) 분류 경계선 |
| A분류 1건 | A | A분류 자체가 소수 (3건 중 1건 실패) |
| C분류 1건 | C | 건설 분류 내 유사 가이드 혼동 |
| H분류 1건 | H | 세분화된 보건 가이드 구별 실패 |
...
**파일**: `backend/app/integrations/analysis_prompts.py`

- 산안법 전체 조문 구조 (제1편~제3편)가 프롬프트에 포함
- 29개 위험유형별 조문 매칭 가이드 테이블 포함
...
| 영역 | 현재 상태 | 개선 방향 |
|------|-----------|-----------|
| 미매칭 법령 2건 | 제383조, 제356조 | 키워드 매핑 추가 또는 프롬프트 강화 |
| 미매칭 KOSHA 6건 | X, P, E, A, C, H 각 1건 | 분류 경계 가이드에 대한 키워드 매핑 |
